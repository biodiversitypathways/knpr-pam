...
recs_plot <- all_recs |>
  filter(!location == "AC-BAT") |>
  rename("Location" = location) |>
  rename("Day of Year" = julian) |>
  group_by(Location, `Day of Year`) |>
  add_tally() |>
  ungroup() |>
  ggplot(aes(x = `Day of Year`, y = n, fill = Location)) +
