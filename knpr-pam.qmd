...
#| label: aru-monitoring-locations
...
## Relative abundance trends of birds for ecological integrity monitoring
...
#| label: relative-abundance-auriol2
#| cap-location: margin
...
#   summarise(mean_individuals = mean(individual_count, na.rm = TRUE))
...
# ggplot(total_per_year, aes(x = Year, y = mean_individuals)) +
#   geom_line(size = 1) +
#   geom_point(size = 2) + 
#   labs(
...
plot_mdm |>
  ...
  scale_colour_viridis_d() +
  scale_x_continuous(labels = scales::number_format(accuracy = 1)) +  # Remove decimals
  theme_bw() +
...
ggplot(count_annual_mdm, aes(x = as.factor(year), y = sum_year, fill = factor(year))) +
  ...
  scale_fill_viridis_d() +
  labs(
    title = "Annual Count of Birds per Location on Auriol Trail",
    x = "Location",
